...
def sarsa(env, num_episodes, discount_factor=1.0, alpha=0.5, epsilon=0.1, verbose=False):
    """
    ...
    for i_episode in range(num_episodes):
        ...
        while not done:
            ...
            probs = policy(n_state)
            n_action = np.random.choice(np.arange(len(probs)), p=probs)

            td_target = reward + discount_factor * Q[n_state][n_action]
